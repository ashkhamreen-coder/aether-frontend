...
const Submit = Platform.OS === 'web' ? 'button' : Pressable;

export function AuthScreen({ mode='signin', initialEmail='', onComplete, navigate }) {
  const [name,setName]=useState(''), [email,setEmail]=useState(initialEmail), [password,setPassword]=useState(''), [confirm,setConfirm]=useState(''), [visible,setVisible]=useState(false), [busy,setBusy]=useState(false), [error,setError]=useState('');
  const submitting=useRef(false), emailRef=useRef(null), passwordRef=useRef(null); const signup=mode==='signup';
  async function submit(event) {
    event?.preventDefault?.();
    if (submitting.current) return;
    const validation=validateAuth({signup,email,password,confirm,name});
    if (validation) { setError(validation); (validation.includes('email') ? emailRef : passwordRef).current?.focus?.(); return; }
    submitting.current=true; setBusy(true); setError('');
    try {
      const session=signup ? await signUp({name,email,password}) : await signIn(email,password);
      onComplete(session.user,signup);
    } catch(e) {
      if (!signup && (e.status===401 || e.status===403)) setError('Email or password is incorrect.');
      else if (signup && e.status===409) setError('An account with this email already exists. Sign in instead.');
      else if (signup && e.status===400) setError(e.message || 'Check your account details and try again.');
      else if (e.code==='TIMEOUT') setError('The request timed out while Ripple was waking up. Please try again.');
      else if (e.code==='NETWORK_ERROR') setError('Ripple could not reach the authentication service. Check your connection and try again.');
      else if (e.status >= 500) setError('Ripple authentication is temporarily unavailable. Please try again.');
  ...
  return <ScrollView keyboardShouldPersistTaps="handled" automaticallyAdjustKeyboardInsets contentContainerStyle={s.page}>
    <View style={s.authHeader}><NavLink href="/" navigate={navigate}><Text style={s.logo}>RIPPLE</Text></NavLink><NavLink href="/" navigate={navigate}><Text style={s.back}>Back to welcome</Text></NavLink></View>
    <Form {...formProps} style={s.card}><Text accessibilityRole="header" style={s.title}>{signup?'Create your account':'Welcome back'}</Text><Text style={s.intro}>{signup?'Start with the Free plan. You can choose a paid plan later.':'Sign in to sync your list and continue watching.'}</Text>
      {signup?<Field label="Name" id="name"><TextInput nativeID="name" accessibilityLabel="Name" type="text" name="name" autoComplete="name" required value={name} onChangeText={setName} style={s.input}/></Field>:null}
      ...
      <Submit accessibilityRole="button" type={Platform.OS==='web'?'submit':undefined} disabled={busy} onPress={Platform.OS==='web'?undefined:submit} style={busy?{...s.submit,...s.disabled}:s.submit}><Text style={s.submitText}>{busy?(signup?'Creating account…':'Signing in…'):signup?'Create account':'Sign in'}</Text></Submit>
      <NavLink href={signup?'/signin':'/signup'} navigate={navigate} style={s.link}><Text style={s.linkText}>{signup?'Already have an account? Sign in':'New to Ripple? Create account'}</Text></NavLink>{!signup?<NavLink href="/forgot-password" navigate={navigate} style={s.link}><Text style={s.linkText}>Forgot password?</Text></NavLink>:null}
    </Form></ScrollView>;
}
function NavLink({ href, navigate, style, children }) {
  if (Platform.OS === 'web') return <a href={href} onClick={event=>{event.preventDefault();navigate(href);}} style={style}>{children}</a>;
  return <Pressable accessibilityRole="link" onPress={()=>navigate(href)} style={style}>{children}</Pressable>;
}
