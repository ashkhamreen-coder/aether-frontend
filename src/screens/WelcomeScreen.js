...
export const PLAN_FALLBACK_COPY = 'Premium plans are temporarily unavailable. You can continue exploring Ripple.';
...
function Heading({ level, children, style, ...props }) {
  if (Platform.OS === 'web') return React.createElement(`h${level}`, { ...props, style:StyleSheet.flatten(style) }, children);
...
function EmailCta({ email, setEmail, error, setError, getStarted, repeated = false }) {
  ...
  return <View style={[s.ctaWrap, repeated && s.ctaCentered]}>
    <View accessibilityRole="form" aria-label="Create your Ripple account" style={s.form}>
      ...
      <Pressable accessibilityRole="button" onPress={getStarted} style={({focused})=>[s.primary,focused&&s.focus]}><Text style={s.primaryText}>Get Started</Text></Pressable>
    </View>
...
export function WelcomeScreen({ navigate }) {
  ...
  useEffect(() => {
    ...
    api('/api/subscriptions/plans').then(value=>{
      if (!live) return;
      const normalized = listPlans(value).filter(Boolean).map(normalizeWelcomePlan);
  ...
  const visibleMontage = conserveData ? montage.slice(0,3) : montage;
  return <View accessibilityRole="main" style={s.page}>
    <LinearGradient accessibilityRole="banner" colors={['#05030d','#100820','#05030d']} start={{x:0,y:0}} end={{x:1,y:1}} style={s.heroShell}>
      ...
      <View style={[s.hero,width<700&&s.heroMobile]}>
        <Heading level={1} testID="welcome-headline" style={[s.title,width<360?s.titleSmall:width<700?s.titleMobile:width<1100?s.titleTablet:s.titleDesktop]}>Entertainment, reimagined.</Heading>
