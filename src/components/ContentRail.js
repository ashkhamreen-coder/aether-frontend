...
export function ContentRail({ row, onOpen, technical = false, first = false }) {
  ...
  const sectionGap = first ? tokens.rail.featuredGap[breakpoint] : tokens.rail.sectionGap[breakpoint];
  const headingGap = tokens.rail.headingGap[breakpoint];
  ...
  return <View
    style={[s.section, { marginTop: sectionGap }, technical && s.technical]}
    onPointerEnter={() => setActive(true)}
    onPointerLeave={() => setActive(false)}
  >
    <View style={[s.head, { paddingHorizontal: gutter, marginBottom: headingGap }]}>
...
const s = StyleSheet.create({
  ...
  rail: { paddingBottom: tokens.rail.bottomPadding, scrollSnapType: 'x proximity', overscrollBehaviorInline: 'contain' },
